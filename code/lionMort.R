...
thetaStart <- matrix(c(-7, 0.001), nrow = ncovs, ncol = defPars$length, 
...
pdf("results/trace004.pdf", width = 10, height = 10)
par(mfrow = c(2, 2))
for (i in 1:4) plot(parMat[, i], type = 'l', main = thetaNames[i])
dev.off()

pdf("results/parDens004.pdf", width = 10, height = 10)
par(mfrow = c(2, 2))
for (i in 1:4) plot(density(parMat[-c(1:1000), i]), type = 'l', 
                    main = thetaNames[i], lwd = 3)
dev.off()

pdf("results/parDens004.pdf", width = 6, height = 8)
par(mfrow = c(2, 1))
for (i in 1:2) {
  plot(density(parMat[-c(1:1000), i]), type = 'l', 
       main = thetaNames[i], lwd = 3, xlim = range(parMat[-c(1:1000), c(i, i+2)]))
  lines(density(parMat[-c(1:1000), i + 2]), col = 'dark red', lwd = 3)
}
dev.off()
